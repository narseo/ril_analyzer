...
public class LogFragment extends Fragment {
  ...
  private static String logfile = "/sdcard/networklog.txt";
  ...
  @Override
    public void onCreate(Bundle savedInstanceState)
    {
      ...
      try {
        ...
        Log.w(TAG, "Exception getting max log entries: " + e.getMessage(), e);
  ...
  public void onNewLogEntry(final LogEntry entry) {
    Log.i(TAG, "SPC_NetworkLog2 - LogFragment.java: New entry! Destination: "+entry.dst);
    if(listDataBuffer == null) {
      return;
    }

    Log.i(TAG, "SPC_NetworkLog2 - LogFragment.java: Uid: "+entry.uid+" - "+entry.uidString);
    

    if(!entry.isValid()) {
      Log.e(TAG, "INVALID ENTRY - NOT LOGGED!");
      return;
    }

    Log.i(TAG, "LogFragment - > newEntry!. Searching for uid: " + entry.uidString);
    ApplicationsTracker.AppEntry appEntry = ApplicationsTracker.uidMap.get(entry.uidString);

    //Narseo: Added to log entry on sdcard. 
    //    
    ...
    Log.i(TAG, "SPC_NetworkLog2 - LogFragment.java. AppName/UID: "+appName+"/"+entry.uid);
    ...
    if(logWriter != null) {
      //Narseo: moved from NetworkLogService. Now should be modified to include
      //all the TCP flags as well
      //
      logWriter.println(entry.timestampProcessing+","+entry.timestamp+","+appName+","+packageName+","+entry.in+","+entry.out+","+entry.proto+","+entry.src+","+entry.dst+","+entry.spt+","+entry.dpt+","+entry.len+","+entry.seqNum+","+entry.ackSeq+","+entry.win+","+entry.res+","+entry.doff+","+entry.fin+","+entry.syn+","+entry.rst+","+entry.psh+","+entry.ack+","+entry.urg+","+entry.ece+","+entry.cwr);
      logWriter.flush();
    }
    //Narseo: - end of modified code. Afterwards is just the code to process
    //and add the entry for the original GUI



    if(appEntry == null) {
      Log.i(TAG, "LogFragment: No appEntry for uid " + entry.uid );      
      return;
    }
    ...
    Log.d(TAG, "LogFragment FOUND!: NewLogEntry: [" + item.app.uidString + "] in=" + item.in + " out=" + item.out + " " + item.srcAddr + ":" + item.srcPort + " --> " + item.dstAddr + ":" + item.dstPort + " proto=" + item.proto + " len=" + item.len);
    ...
    synchronized(listDataBuffer) {
      ...
      while(listDataBuffer.size() > maxLogEntries) {
        Log.d(TAG, "Log buffer size reached maxLogEntries limit; truncating");
